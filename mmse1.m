...
function [X_estimado] = mmse1(y,x,v, fs) % *o arquivo e a função devem ter o mesmo nome
...
for n=1:Nframes
    ...
    vk=ksi.*gammak./(1+ksi); %encontro o vk, eq12 Artigo Ephraim
...
[j1]=besseli(1,vk/2);
        ...
        C=exp(-0.5*vk);
        A=((c*(vk.^0.5)).*C)./gammak;
        B=(1+vk).*j0+vk.*j1; %j0 e j1 s�o as fun��es modificadas de Bessel de zero e primeira ordem
        hw=A.*B; 
...
%wavwrite(xfinal,Srate,16,outfile); %ORIGINAL
%outfile = 'C:\Users\Lu_Xa\Desktop\Reunioes\12_02_2020\MMSE_TESTE\Resultado\nome_arquivo.wav';
% audiowrite(outfile,xfinal,Srate);

X_estimado = xfinal; %CORRETOOOOOOO


% cd('./sinais_DepoisTecnicas/MMSE/')
% audiowrite(outfile,xfinal,Srate);
% X_estimado = xfinal; 
% cd('./../..')
