function [ratio, energy] = SRMR_CI2(s,vad,varargin)
...
% modified on October 20th, by Rafael Chiea: input s is a nchanels X timesamples 
% matrix, which is the output of the CIfilter  
...
if ischar(s)
    ...
    if isempty(varargin)
        error('Second argument must be the sampling rate if inputs are vectors');
    else
...
%% Cochlear filterbank/envelope computation
    ...
    % Altered by R.C. 10/2017. Input is the filtered signal. 
    [~, bandwidths] = CIFilterBank(fs, nCochlearFilters, lowFreq, zeros(100,1));
...
aux = sum(vad);
energy = zeros(nCochlearFilters, nModFilters, ceil(aux/wInc));
    
for k=1:nCochlearFilters
    idx = find(s(k,:).^2>max(abs(s(k,:)))^2/(10^(50/10)),1);
    if isempty(idx);
        ratio=0;
        energy=0;
        return;
    ...
    %%
    modulationOutput = modulationFilterBank(tempEnv, modFilterCFs, fs, 2);
    for m=1:nModFilters
