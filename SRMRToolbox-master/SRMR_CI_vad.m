function [ratio, energy] = SRMR_CI_vad(s,vad,varargin)
%SRMR Computes the SRMR-CI score of the given signal.
% Modifyied by R. Chiea, 10/2017 - ideal vad input included
...
% vad: ideal vad array, with the same length of input signal (1 = voice on; 0 = voice off).
...
%% adicionado por mim para evitar problemas com o VAD
idx = find(s.^2>max(abs(s))^2/(10^(50/10)),1);
if isempty(idx);
    ratio=0;
    energy=0;
    return;
...
%[s, fs] = preprocess2(s, fs); %vad from the voicebox toolbox
[s, fs] = preprocess3(s, fs, vad); %ideal vad from input
